...
const ink = (() => {
  ...
  function ensureCanvasSize() {
    ...
    // Keep a neutral transform on the backing canvas; drawing uses world coords
...
// csvToGrid uses SheetJS to parse CSV text into a grid (array of arrays)
...
/*
  ...
  - expects the Spells CSV (gid=0) exported
  - extracts spell rows for Sorcerer and Wizard blocks and stores them into state.data.spells
  - also attempts to parse slot tables if present (slot info may be separate)
*/
function ingestSpellsFromGrid(grid) {
  try {
    ...
    for (let r = 0; r < Math.min(grid.length, 40); r++) {
      const row = (grid[r] || []).map(c => norm(c)).join("|");
      if (row.includes("sorcerer") && sorcHeader < 0) sorcHeader = r;
      if ((row.includes("wizard") || row.includes("evoker")) && wizHeader < 0) wizHeader = r;
      if (row.includes("spell") && row.includes("sl")) {
        // if both not set, set them heuristically
        if (sorcHeader < 0) sorcHeader = r;
        else if (wizHeader < 0) wizHeader = r;
      }
    }

    // Helper: find spell name column by scanning next rows for text
    function findSpellCol(headerRow, preferredCol) {
      ...
      // fallback: first non-empty column after header
    ...
    function readBlock(headerRow, mode) {
      ...
      const preferred = map["sorcerer"] ?? map["wizard"] ?? map["spell"] ?? null;
      ...
      // optional columns
      const colType = map["type"] ?? -1;
      const colEvo = map["evo?"] ?? -1;
      const colFire = map["fire?"] ?? -1;
      const colRange = map["range"] ?? -1;
      const colArea = map["area"] ?? -1;
      const colDamage = map["damage"] ?? -1;
      const colDuration = map["duration"] ?? -1;
      const colPrep = map["preparations"] ?? -1;
      const colNotes = map["notes"] ?? -1;

      const rows = [];
      if (colSpell == null) return rows;
      for (let r = headerRow + 1; r < grid.length; r++) {
        const name = String((grid[r] || [])[colSpell] || "").trim();
        if (!name) break;
        const slRaw = colSL >= 0 ? String((grid[r] || [])[colSL] || "").trim() : "";
        ...
        rows.push({
          ...
          url: "",
          sl,
          type: colType >= 0 ? String((grid[r] || [])[colType] || "") : "",
          evo: colEvo >= 0 ? (String((grid[r] || [])[colEvo] || "").trim() === "1") : false,
          fire: colFire >= 0 ? (String((grid[r] || [])[colFire] || "").trim() === "1") : false,
          range: colRange >= 0 ? String((grid[r] || [])[colRange] || "") : "",
          area: colArea >= 0 ? String((grid[r] || [])[colArea] || "") : "",
          damage: colDamage >= 0 ? String((grid[r] || [])[colDamage] || "") : "",
          duration: colDuration >= 0 ? String((grid[r] || [])[colDuration] || "") : "",
          notes: colNotes >= 0 ? String((grid[r] || [])[colNotes] || "") : "",
          prep: mode === "wiz" && colPrep >= 0 ? String((grid[r] || [])[colPrep] || "") : ""
        });
      }
      return rows;
    }
...
function renderSlots() {
  ...
  sorTitle.textContent = `Sorcerer slots (effective level ${calc.meta?.sorcererLevel ?? effSorc})`;
  ...
  wizTitle.textContent = `Wizard slots (effective level ${calc.meta?.wizardLevel ?? effWiz})`;
